...
import { collection, query, onSnapshot, doc, updateDoc, serverTimestamp, orderBy, writeBatch } from 'firebase/firestore';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import * as XLSX from 'xlsx';
import { toast } from 'react-toastify';
import { FiX, FiDownload, FiPrinter, FiPauseCircle, FiPlayCircle, FiCheck, FiSearch } from 'react-icons/fi';
...
const PainelAnalista = () => {
    ...
    const handleExportarELimpar = async () => {
        try {
            ...
            await fetch(WEB_APP_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ tipo: "CHAMADOS_POWERBI", dados }) });
        ...
        <div className="meus-chamados-container">
            <header className="page-header">
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <button onClick={() => setMostrarBuscaImpressao(true)} className="btn-print-top">
                        <FiPrinter /> Imprimir OS
                    </button>
                    
                    {userData?.role === 'adm' && (
                        !aguardandoConfirmacao ? 
                        <button onClick={() => setAguardandoConfirmacao(true)} className="btn-action-remove">
                            <FiDownload /> Exportar e Limpar
                        </button> :
                        <div className="confirm-action-box">
                            <span>Excluir fechados?</span>
                            <button onClick={handleExportarELimpar} className="btn-confirm-yes">Sim</button>
                            <button onClick={() => setAguardandoConfirmacao(false)} className="btn-confirm-no">Não</button>
                        </div>
                    )}
                </div>
            </header>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            chamados.map((item) => (
                                <tr 
                                    key={item.id} 
                                    onClick={() => { setChamadoSelecionado(item); setMostrarDetalhes(true); }}
                                    className="row-hover-effect"
                                    style={{ cursor: 'pointer' }}
                                >
                                    <td><strong>#{item.numeroOs}</strong></td>
                                    <td>{formatarDataHora(item.criadoEm)}</td>
                                    <td>
                                        <strong>{item.nome}</strong>
                                        <br />
                                        <small style={{ color: '#007bff', fontWeight: 'bold', textTransform: 'uppercase' }}>
                                            {item.setor || "Geral"}
                                        </small>
                                    </td>
                                    <td style={{ fontSize: '0.85rem', color: '#666' }}>{truncarTexto(item.descricao)}</td>
                                    <td><span className={`status-badge ${item.status}`}>{item.status.toUpperCase()}</span></td>
                                    <td onClick={(e) => e.stopPropagation()}>
                                        <div style={{ display: 'flex', gap: '5px' }}>
                                            {item.status === 'aberto' && (
                                                <button onClick={() => handleAssumirChamado(item)} className="btn-assumir">Assumir</button>
                                            )}
                                            {item.status === 'em atendimento' && (
                                                <>
                                                    <button onClick={() => { setChamadoParaPendente(item); setMostrarModalPendencia(true); }} className="btn-pausar" title="Pausar"><FiPauseCircle /></button>
                                                    <button onClick={() => { setChamadoParaFinalizar(item); setMostrarModal(true); }} className="btn-finalizar-small" title="Finalizar"><FiCheck /></button>
                                                </>
                                            )}
                                            {item.status === 'pendente' && (
                                                <button onClick={() => handleRetomarChamado(item)} className="btn-retomar"><FiPlayCircle /> Retomar</button>
                                            )}
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div className="modal-content" style={{ maxWidth: '400px' }}>
                        ...
                        <form onSubmit={handleBuscarEImprimir}>
                            ...
                            <input 
                                ...
                                autoFocus
                            />
                            <button type="submit" className="btn-salvar-modern" style={{ width: '100%' }}>
            ...
            {/* MODAL DE DETALHES - ATUALIZADO COM MOTIVO DA PAUSA */}
            {mostrarDetalhes && (
                <div className="modal-overlay">
                    <div className="modal-content">
                        ...
                        <div className="detalhes-body">
                            ...
                            {/* ALERTA DE MOTIVO DA PAUSA */}
                            {chamadoSelecionado?.status === 'pendente' && (
                                <div className="alerta-pausa" style={{ backgroundColor: '#fff3cd', borderLeft: '5px solid #ffc107', padding: '15px', marginBottom: '20px', borderRadius: '4px', color: '#856404' }}>
                                    <strong style={{ display: 'block', marginBottom: '5px' }}>⚠️ MOTIVO DA PENDÊNCIA (SLA PAUSADO):</strong>
                                    <p style={{ margin: 0, fontWeight: '500' }}>{chamadoSelecionado?.motivoPausa || "Nenhum motivo detalhado."}</p>
                                    <small>Pausado em: {formatarDataHora(chamadoSelecionado?.pausadoEm)}</small>
                                </div>
                            )}
                            ...
                            <div className="info-grid">
                                ...
                                <p><strong>Setor:</strong> <span style={{textTransform: 'uppercase', color: '#007bff', fontWeight: 'bold'}}>{chamadoSelecionado?.setor || "Não informado"}</span></p> 
                                <p><strong>Status:</strong> <span className={`status-badge ${chamadoSelecionado?.status}`}>{chamadoSelecionado?.status?.toUpperCase()}</span></p>
                                <p><strong>Técnico:</strong> {chamadoSelecionado?.tecnicoResponsavel || "Aguardando"}</p>
                            </div>
                            <hr />
                            <h3>Descrição do Usuário</h3>
                            <div className="descricao-box-full" style={{ whiteSpace: 'pre-wrap' }}>{chamadoSelecionado?.descricao}</div>
                        </div>
                        <div className="modal-footer-actions">
                            <button onClick={() => setMostrarDetalhes(false)} className="btn-voltar">Fechar</button>
                            
                            {/* Ações Rápidas dentro do Modal */}
                            {chamadoSelecionado?.status === 'pendente' && (
                                <button onClick={() => handleRetomarChamado(chamadoSelecionado)} className="btn-retomar">
                                    <FiPlayCircle /> Retomar Agora
                                </button>
                            )}

                            {chamadoSelecionado?.status === 'em atendimento' && (
                                <button onClick={() => { setMostrarDetalhes(false); setChamadoParaFinalizar(chamadoSelecionado); setMostrarModal(true); }} className="btn-finalizar-detalhe">Finalizar Agora</button>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="modal-overlay">
                    <div className="modal-content">
                        ...
                        <form onSubmit={handleColocarPendente}>
                            <textarea className="form-input" value={motivoPendencia} onChange={(e) => setMotivoPendencia(e.target.value)} required placeholder="Descreva o motivo da pendência (Ex: Aguardando peça, Aguardando retorno do usuário...)" />
                            <div className="modal-footer-actions">
                                <button type="button" onClick={() => setMostrarModalPendencia(false)} className="btn-voltar">Cancelar</button>
                                <button type="submit" className="btn-salvar-modern">Confirmar Pausa</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className="modal-overlay">
                    <div className="modal-content">
                        ...
                        <form onSubmit={handleFinalizarChamado}>
                            ...
                            <input type="text" className="form-input" value={patrimonio} onChange={(e) => setPatrimonio(e.target.value)} required />
                            <label>Parecer Técnico</label>
                            <textarea className="form-input" value={parecerTecnico} onChange={(e) => setParecerTecnico(e.target.value)} required />
                            <div className="modal-footer-actions">
                                <button type="button" onClick={() => setMostrarModal(false)} className="btn-voltar">Cancelar</button>
                                <button type="submit" className="btn-salvar-modern">Encerrar</button>
                            </div>
                        </form>
                    </div>
                </div>
